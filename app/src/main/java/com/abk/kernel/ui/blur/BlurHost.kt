 ...
 *
 * Wrap a screen's [androidx.compose.material3.Scaffold] in this composable. The screen
 * attaches [Modifier.blurSource] to its **body** content root (the part that scrolls
 * beneath the bars) so it is captured as the blur source, and passes `enableBlur = true`
 * to its top bar so the bar applies [Modifier.blurEffect].
 *
 ...
 * @param content Screen content. May read [LocalBlurState].
 */
@Composable
fun BlurHost(
