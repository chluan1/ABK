...
class PreferencesRepository(private val context: Context) {
    ...
    val customBackgroundUri: Flow<String?> = context.dataStore.data.map { it[KEY_CUSTOM_BACKGROUND_URI] }.distinctUntilChanged()
    val backgroundImageEnabled: Flow<Boolean> = context.dataStore.data.map { it[KEY_BACKGROUND_IMAGE_ENABLED] ?: false }.distinctUntilChanged()
    val uiSurfaceAlpha: Flow<Float> = context.dataStore.data
        .map { it[KEY_UI_SURFACE_ALPHA] ?: 1f }
        .distinctUntilChanged()
    val blurEnabled: Flow<Boolean> = context.dataStore.data.map { it[KEY_BLUR_ENABLED] ?: true }
    val blurBackgroundExpEnabled: Flow<Boolean> = context.dataStore.data.map {
        it[KEY_BLUR_BACKGROUND_EXP_ENABLED] ?: false
    }
